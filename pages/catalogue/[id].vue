      ...
      <div class="flex justify-between items-center">
        <div class="w-[-800px] h-[400px] rounded-xl">
          <img :src="`${useRuntimeConfig().public.API_URL}/images/${detailProduct.image
            }`" :alt="detailProduct.name" class="rounded-xl drop-shadow-xl object-cover h-full w-full" />
        </div>
        <div class="flex flex-col justify-between gap-10">
          ...
          <div @click="handleTransaction"
            ...
            <p class="text-2xl text-white font-primary font-light flex items-center justify-center">
              Rp. {{ detailProduct.price }},-
            </p>

            <button @click="handleTransaction">
              <svg class="fill-white" height="60" width="60" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
...
const { data: dataProduct } = await useFetch(
  `${config.public.API_URL}/product/${routeParams.id}`,
  {
    onResponse({ response }) {
...
const handleTransaction = () => {
  transactionStore.data.id_product = detailProduct.value._id;
  transactionStore.data.total_price = detailProduct.value.price;
